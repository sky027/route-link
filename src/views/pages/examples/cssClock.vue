  ...
  methods: {
    initClock() {
      ...
      function initNumXY(){
        // 1、12个数字的位置设置
        let radius = 160;//大圆半价
        let dot_num = 360/$(".dot").length;//每个div对应的弧度数
        //每一个dot对应的弧度;
        let ahd = dot_num*Math.PI/180;
      ...
      initNumXY();//调用上面的函数
      //获取时钟id
      let hour_line = document.getElementById("hour_line"),
      ...
      function setTime(){
        ...
        //时分秒针设置
        let hour_rotate = (hours*30-90) + (Math.floor(minutes / 12) * 6);
